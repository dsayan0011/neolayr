    ...
    $.ajax({
        ...
        if (data == 0) {
            ...
            if (is_discounted == false) {
                ...
                is_discounted = true;
				ShowNotificator('alert-success', "Discount Code Applied");
            }
        }
    });
}
...
function manageShoppingCart(action, article_id, reload, action_type="") {
    ...
    $.ajax({
        ...
        if (reload == true) {
            ...
            if(action_type!=""){
                 location.href = reload+"?variant="+article_id;
                 return;
            }else{
...
function clearCart() {
    $.ajax({type: "POST", url: variable.clearShoppingCartUrl}).done(function (data){location.reload();});
