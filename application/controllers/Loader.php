...
class Loader extends MY_Controller
{
    ...
    {
        $template = $this->template;
        $style = file_get_contents(VIEWS_DIR . $template . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'fonts' . DIRECTORY_SEPARATOR . $file);
        if (!$style) {
            header('HTTP/1.1 404 Not Found');
            return;
        }
     ...
     public function images($file, $template = null)
    {
        if ($template == null) {
            $template = $this->template;
        } else {
        ...
        $path = VIEWS_DIR . $template . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . $file;
        $img = file_get_contents($path);
        if (!$img) {
            header('HTTP/1.1 404 Not Found');
            return;
    ...
    public function fancyboxCss($file)
    {
        $template = $this->template;
        $style = file_get_contents(VIEWS_DIR . $template . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'fancybox' . DIRECTORY_SEPARATOR . $file);
        if (!$style) {
            header('HTTP/1.1 404 Not Found');
            return;
        }
    ...
    public function fancyboxJS($file)
    {
        $template = $this->template;
        $style = file_get_contents(VIEWS_DIR . $template . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'fancybox' . DIRECTORY_SEPARATOR . $file);
        if (!$style) {
