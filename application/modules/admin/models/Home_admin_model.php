...
class Home_admin_model extends CI_Model
{
	...
	public function newReturnOrdersCheck()
    {
        $result = $this->db->query("SELECT count(return_update_id) as num FROM `order_return_update` WHERE viewed = 0");
        $row = $result->row_array();
        return $row['num'];
    }
	public function newUsersCheck()
    {
        $result = $this->db->query("SELECT count(id) as num FROM `users_public` WHERE viewed = 0");
        $row = $result->row_array();
        return $row['num'];
    }
	...
	function getUserPermission($level)
	{		
		$query = $this->db->get_where('user_levels_permissions', array('user_level_id' => $level));
    ...
    {          
        ...
        $this->db->select('products_translations.id as productsTranslationsID, products.id as productID, products_translations.title as productTitle,products.sku,products.product_title');
