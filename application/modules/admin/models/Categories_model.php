...
class Categories_model extends CI_Model
{
    ...
    {
        $this->db->trans_begin();
        if (!$this->db->insert('shop_categories', array('sub_for' => $post['sub_for'],'tag' => $post['categorie_tag']))) {
            log_message('error', print_r($this->db->error(), true));
        }
        ...
        foreach ($post['translations'] as $abbr) {
			...
			$arr['category_slug'] =url_title($post['categorie_name'][$i], 'dash', true);
            $arr['for_id'] = $id;
    ...
    public function editShopCategorie($post)
    {
        ...
        if (!$this->db->update('shop_categories_translations', array(
                    'name' => $post['name'],
					'category_slug' => url_title($post['name'], 'dash', true)
                ))) {
            log_message('error', print_r($this->db->error(), true));
            show_error(lang('database_error'));
        }
	...
	public function editShopCategorieTag($post)
    {
        $this->db->where('id', $post['editid']);
        if (!$this->db->update('shop_categories', array(
                    'tag' => $post['new_tag']
