...
<form method="POST">
 <input type="hidden" name="edit" value="<?= isset($vendor_details) ? $vendor_details['id'] : '0' ?>">
                        ...
                        <div class="form-group">
                            <label for="name">Nickname(Max 8 Character)</label>
                            <input type="text" name="nickname" maxlength="8" required value="<?= isset($vendor_details['nickname']) ? $vendor_details['nickname'] : '' ?>" class="form-control" id="nickname">
                        </div>
                         <div class="form-group">
                       ...
                       <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="notify">User Level</label>
                            <select name="level" class="form-control">
                            	<?php foreach($users_level->result() as $level){
									$display_level = true;
								    if($this->session->userdata('level_id')!='1' && $level->user_level_id == "1")
								    $display_level = false;
								 
								    if($display_level){
									?>
                            	<option <?php if(isset($vendor_details['level']) && ($vendor_details['level'] == $level->user_level_id)) echo 'selected="selected"';?> value="<?php echo $level->user_level_id;?>"><?php echo $level->user_level_name;?></option>
                                <?php } } ?>
                            </select>
                        </div>
                          <div class="form-group">
                            ...
                            <select name="vendor_status" id="vendor_status" class="form-control" required>
                            	<option <?php if(isset($vendor_details['vendor_status']) && ($vendor_details['vendor_status'] == 1)) echo 'selected="selected"';?> value="1">Active</option>
                                <option <?php if(isset($vendor_details['vendor_status']) && ($vendor_details['vendor_status'] == 0)) echo 'selected="selected"';?> value="0">Inactive</option>
                            </select>
                        </div>
