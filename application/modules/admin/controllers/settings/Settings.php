...
class Settings extends ADMIN_Controller
{
    ...
    public function index()
    {
        ...
        $data['footerSocialInstagram'] = $this->Home_admin_model->getValueStore('footerSocialInstagram');        
    ...
    private function postChecker()
    {
		...
		if (isset($_POST['facebook_login'])) {
            $this->Home_admin_model->setValueStore('facebook_login', $_POST['facebook_login']);
            $this->session->set_flashdata('facebook_login', 'Facebbok Login Status changed');
            $this->saveHistory('Facebbok Login Status changed');
            redirect('admin/settings');
        }
		if (isset($_POST['google_login'])) {
            $this->Home_admin_model->setValueStore('google_login', $_POST['google_login']);
            $this->session->set_flashdata('google_login', 'Google login status changed');
            $this->saveHistory('Google login status changed');
            redirect('admin/settings');
        }
